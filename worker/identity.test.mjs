...
test('manual links preserve raw visits and can be separated', async () => {
  ...
  assert.deepEqual((await stats()).profiles.find(p => p.id === A).oss.sort(), ['Android', 'iOS']);
  assert.deepEqual((await stats()).profiles.find(p => p.id === A).browsers.sort(), ['Brave', 'Chrome']);
  ...
  const visits = await (await request('/api/profile-visits?profile=' + A)).json();
  assert.equal(visits.visits.length, 2);
  assert.equal(visits.visits[0].city, 'Bengaluru');
  assert.equal((await request('/api/move-visit', { viewId: 2, fromProfile: A, targetProfile: 'new' }, 'https://attacker.example')).status, 403);
  const moved = await (await request('/api/move-visit', { viewId: 2, fromProfile: A, targetProfile: 'new' })).json();
  assert.equal(moved.ok, true);
  assert.equal((await stats()).totals.unique, 4);
  assert.equal((await stats()).profiles.find(p => p.id === A).visits, 1);
  assert.equal((await stats()).profiles.find(p => p.id === moved.profileId).visits, 1);
  assert.equal(sqlite.prepare('SELECT visitor_id FROM page_views WHERE id = 2').get().visitor_id, A);
  assert.equal(sqlite.prepare('SELECT visitor_id FROM page_engagement WHERE id = 1').get().visitor_id, A);
  assert.equal((await (await request('/api/restore-visit', { viewId: 2, profileId: moved.profileId })).json()).ok, true);
  assert.equal((await stats()).totals.unique, 3);
  assert.equal(sqlite.prepare('SELECT COUNT(*) AS n FROM visitor_visit_events').get().n, 2);
